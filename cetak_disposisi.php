<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        <div class="container">
            <div id="colres">
                <div class="disp">';
                    $query2 = mysqli_query($config, "SELECT  nama, status, alamat, logo FROM tbl_instansi");
                    list( $nama, $status, $alamat, $logo) = mysqli_fetch_array($query2);
                    if(!empty($logo)){
                    ...
                    if(!empty($status)){
                        ...
                        echo '<h6 class="status"> Lembaga Pemerintahan </h6>';
                    }
                    if(!empty($alamat)){
                        echo '<span id="alamat">'.$alamat.'</span>';
                    } else {
                        echo '<span id="alamat">Jl. Letjen M.T. Haryono No.11, RT.1/RW.6, Cawang, Kec. Kramat jati, Kota Jakarta Timur, Daerah Khusus Ibukota Jakarta 13630 </span>';
                    }
                    echo '
                </div>
            ...
            <div id="lead">
                <p>Kepala Biro</p>
                <div style="height: 50px;"></div>';
                $query = mysqli_query($config, "SELECT karo, nip FROM tbl_instansi");
                list($karo,$nip) = mysqli_fetch_array($query);
                if(!empty($karo)){
                    echo '<p class="lead">'.$karo.'</p>';
                } else {
                    echo '<p class="lead">Kepala Biro</p>';
                }
                if(!empty($nip)){
                    echo '<p>NIP. '.$nip.'</p>';
                } else {
